...
bool InitCommandLine(int argc, char** argv, po::variables_map* conf) {
  ...
  opts.add_options()
    ...
    ("beta_scale,g","Scale beta by loss")
    ("max_step_size,C", po::value<double>()->default_value(0.01), "regularization strength (C)")
    ("max_bstep_size,E", po::value<double>()->default_value(0.01), "regularization strength (for beta)")
...
bool ModelCompareN(const HI& h1, const HI& h2 )
{
  return -1 * h1->oracle_feat_diff.dot(dense_w_local) < -1 * h2->oracle_feat_diff.dot(dense_w_local);
}


bool ModelCompareB(const HI& h1, const HI& h2 )
{
  return ( h1->oracle_feat_diff.dot(dense_w_local)  >  h2->oracle_feat_diff.dot(dense_w_local) );
}
...
vector<shared_ptr<HypothesisInfo> > SelectPairBeta(vector<shared_ptr<HypothesisInfo> >* cur_c, bool loss, int bs, vector<weight_t> dense_weights)
{
  bool DEBUG_SELECT= false;
  vector<shared_ptr<HypothesisInfo> >& cur_constraint = *cur_c;
  vector<shared_ptr<HypothesisInfo> > pair;

  if (cur_constraint.size() < 2) {return pair;}

  double bound_loss = (loss ? cur_constraint[1]->oracle_loss : 1);
  cerr << "Constraint " << bs << " in SelectBeta:" << cur_constraint[1]->oracle_feat_diff.dot(dense_weights) << " " <<  pair.size() << " " <<  cur_constraint[1]->mt_metric << " " << cur_constraint[1]->features.dot(dense_weights) << " " <<  cur_constraint[1]->oracle_loss  << endl;
  if( bs * (cur_constraint[1]->oracle_feat_diff.dot(dense_weights) ) - bound_loss * beta_B - SMO_EPSILON > 0)
    {
...
int main(int argc, char** argv) {
  ...
  beta_scale = conf.count("beta_scale");
  beta_B = conf["beta_b"].as<double>();
  cerr << "Beta " << beta_B << endl;
  ...
  const double max_bstep_size = conf["max_bstep_size"].as<double>();
  ...
  while(*in) {
      ...
      else if(optimizer == 5) //full mira with n-best list of constraints from hope, fear, model best
	{
      ...
      else if(optimizer == 2 || optimizer == 3) //PA and Cutting Plane MIRA update
	  {
	    ...
	    cur_constraint[0].push_back(cur_good_v[0]); //add oracle to constraint set for loss
            cur_constraint[1].push_back(cur_good_v[0]); //add oracle to constraint set for B
            cur_constraint[2].push_back(cur_good_v[0]); //add oracle to constraint set for -B
		...
		if(optimizer == 2){ //PA
		  cur_constraint[0].push_back(cur_bad_v[0]);
		  

		  sort(oracles[cur_sent].bad.begin(),oracles[cur_sent].bad.end(),ModelCompareB);
                  shared_ptr<HypothesisInfo> max_score_diff = oracles[cur_sent].bad[0];
                  double bound_loss = max_score_diff->oracle_loss;
                  cerr << "Bound Constraint " << max_score_diff->oracle_feat_diff.dot(dense_weights) << endl;
                  if(max_score_diff->oracle_feat_diff.dot(dense_weights) - beta_B - SMO_EPSILON > 0)
                    {
                      cur_constraint[1].push_back(max_score_diff);
                      cerr << "Constraint 1 " << max_score_diff->oracle_feat_diff.dot(dense_weights) << " " <<  cur_constraint[1].size()<< " " << max_score_diff->mt_metric << " " << max_score_diff->features.dot(dense_weights) << endl;
                    }


                  sort(oracles[cur_sent].bad.begin(),oracles[cur_sent].bad.end(),ModelCompareN);
                  shared_ptr<HypothesisInfo> min_neg_score_diff = oracles[cur_sent].bad[0];
                  cerr << "Bound Constraint2 " << min_neg_score_diff->oracle_feat_diff.dot(dense_weights) << endl;
                  //bound_loss = min_neg_score_diff->oracle_loss;
                  if(-1 * min_neg_score_diff->oracle_feat_diff.dot(dense_weights) - beta_B - SMO_EPSILON > 0)
                    {
                      cur_constraint[2].push_back(min_neg_score_diff);
                      cerr << "Constraint 2 " << -min_neg_score_diff->oracle_feat_diff.dot(dense_weights)<< endl;
                    }
		  ...
		  { //cutting plane to add constraints
		    ...
		    cut_plane_calls++;
		    CuttingPlane(&cur_constraint[0], &optimize_again, oracles[cur_sent].bad, dense_weights);
		...
		if(optimize_again)
		  {
		    //SMO 
		    for(int u=0;u!=cur_constraint[0].size();u++)	
		      { 
			cur_constraint[0][u]->alpha =0;
		      }
		    cur_constraint[0][0]->alpha = 1;
		    cerr <<" Optimizing with " << cur_constraint[0].size() << " constraints" << endl;
		    int smo_iter = MAX_SMO;
		    int iter =0;
		    while (iter < smo_iter)
		      {			
			//select pair to optimize from constraint set
			vector<shared_ptr<HypothesisInfo> > cur_pair = SelectPair(&cur_constraint[0]);
			double delta=0;
			...
			if(!cur_pair.empty()){
			delta = ComputeDelta(&cur_pair,max_step_size, dense_weights);
			...
			vector<shared_ptr<HypothesisInfo> > cur_beta_pair = SelectPairBeta(&cur_constraint[1],beta_scale,1, dense_weights);

			if (!cur_beta_pair.empty())
			  //update weights based on bounding constraint
			  //if(cur_constraint[1].size() > 1)
                          {
                            cerr<<"Updating bound " << cur_beta_pair[0]->features.dot(dense_weights) - cur_beta_pair[1]->features.dot(dense_weights)<< endl;
                            double bound_margin = cur_beta_pair[1]->features.dot(dense_weights) - cur_beta_pair[0]->features.dot(dense_weights);
                            SparseVector<double> bound_diff = cur_beta_pair[0]->features;
                            bound_diff -= cur_beta_pair[1]->features;
                            double bound_diffsqnorm = bound_diff.l2norm_sq();
                            double bound_loss = (beta_scale ? cur_beta_pair[1]->oracle_loss : 1);
                            //   double bound_update = (cur_constraint[1][1]->oracle_loss*beta_B + bound_margin)/ bound_diffsqnorm;
                            double bound_update = (bound_loss * beta_B + bound_margin)/ bound_diffsqnorm;
                            cerr << "BU " << bound_update;
                            if ((bound_update) > max_bstep_size) bound_update = max_bstep_size;
                            if ((bound_update) < -max_bstep_size) bound_update = -max_bstep_size;
                            cerr << "-->" << bound_update << endl;

                            lambdas += (cur_beta_pair[0]->features * bound_update);
                            lambdas -= (cur_beta_pair[1]->features * bound_update);

                            dense_weights.clear();
			    lambdas.init_vector(&dense_weights);
			    dense_w_local = dense_weights;
                            
                            cerr << "BA: " << cur_beta_pair[1]->oracle_feat_diff.dot(dense_weights) << " " <<  cur_beta_pair[0]->features.dot(dense_weights) - cur_beta_pair[1]->features.dot(dense_weights)<< endl;
	    ...
	    double max_fear = cur_constraint[0][cur_constraint[0].size()-1]->fear;
	    double temp_objective = 0.5 * lambda_change;// + max_step_size * max_fear;

	    for(int u=0;u!=cur_constraint[0].size();u++)	
	      { 
		cerr << cur_constraint[0][u]->alpha << " " << cur_constraint[0][u]->hope << " " << cur_constraint[0][u]->fear << endl;
		temp_objective += cur_constraint[0][u]->alpha * cur_constraint[0][u]->fear;
	      }
