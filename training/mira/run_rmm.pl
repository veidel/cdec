...
#	my $im1 = $opt_iter - 1;
	...
	my $decoder_cmd = "$decoder -c $iniFile -w $weightsFile $refs_comma_sep -m $metric -s $metric_scale -b $update_size -k $kbest_size -o $optimizer $cur_pass -O $weightdir -D $dir  -h $hopes -f $fears -C $step_size -E $b_step_size -B $beta";
	if($unique_kbest){
		$decoder_cmd .= " -u";
	}
	if($sent_approx){
